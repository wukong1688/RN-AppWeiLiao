/**
 * 使用ScrollView实现的一个ViewPager Banner效果
 *
 */
...
export default class Banner extends Component {
    ...
    startTimer() {
        ...
        this.timer = setInterval(() => {
            ...
            if (curr + 1 > this.props.bannerData.length - 1) {
                curr = 0;
            } else {
                curr++;
            }
    ...
    getImages() {
        var images = [];
        for (var i = 0; i < this.props.bannerData.length; i++) {
            images.push(
                <View key={i}>
                    {<Image source={{uri: this.props.bannerData[i]}} style={styles.image}/>}
                </View>
            );
        }
        return images;
